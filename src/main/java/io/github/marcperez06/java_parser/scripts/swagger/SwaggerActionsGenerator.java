...
import sogeti.testing_framework_base.core.shared.java_parser.objects.swagger.SwaggerEndpointObjectInfo;
...
import sogeti.testing_framework_base.core.shared.properties.EnvironmentType;
import sogeti.testing_framework_base.core.shared.properties.PropertiesManager;
import sogeti.testing_framework_base.core.shared.properties.keys.ApiEnvProperties;
...
public class SwaggerActionsGenerator extends SwaggerAbstractGenerator {
	...
	private void createActionClass(String endpointTag, Map<String, SwaggerEndpoint> endpointMap) {
		...
		String className = Utils.capitalizeWord(tagWithoutErrors) + "Actions";
		String endpointClass = Utils.capitalizeWord(tagWithoutErrors) + "Endpoints";
		...
		parser.createStringConstant("REST_API_IS_NOT_DEFINED", "String", constantValue, Keyword.PRIVATE);
	...
	private void extendActionClass(MyJavaParser parser) {
		...
		if (parentClassName != null && !parentClassName.isEmpty()) {
			...
			parser.addImport("sogeti.testing_framework_base.core.api.actions.BaseApiActions");
	...
	private List<String> getImports() {
		...
		imports.add("sogeti.testing_framework_base.utils.Logger");
		imports.add("sogeti.testing_framework_base.core.api.endpoints.Endpoint");
		imports.add("sogeti.testing_framework_base.core.api.rest.RestApiConfig");
		return imports;
	}
	...
	private String getMethodName(String requestMethod, SwaggerRequestInfo requestInfo) {
		...
		String finalName = requestMethod.toLowerCase() + Utils.capitalizeWord(methodName);
	...
	private List<String> getImportsFromParameters(List<Parameter> parameters) {
		...
		if (parameters != null && !parameters.isEmpty()) {
			...
			for (Parameter param : parameters) {
				...
				if (!Utils.isJavaVariableType(variableType)) {
					if (variableType.contains("Map")) {
						imports.add("java.util.Map");
					} else if (variableType.equals("List")) {
						imports.add("java.util.List");
					} else {
						String importName = this.getObjectPackage(Utils.capitalizeWord(variableType));
	...
	private String getObjectPackage(String className) {
		...
		if (Utils.isReservedKeyword(extendPackage)) {
